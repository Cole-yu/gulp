## 特别说明(important!)
    ...
    第一步：npm install
    第二步：打开node_modules\gulp-assets-rev\index.js文件:修改源代码
...
## 任务操作
    ...
    gulp.task('taskName',function(cb){                  //必须写上cb,
       return runSequence('a',['b','c'],'d',cb);        // 先串行执行a,b和c任务可以并行,再执行d任务,最后函数会调用callback,以通知Gulp引擎
    });
